...
yaml.Nodes preserve comments and locations in the source and enable editing in-place. JSONPointer can be used to
locate the fields to be edited.

This package implements the [JSONPointer standard](https://tools.ietf.org/html/rfc6901) with one extension: it
handles pointers into k8s manifests which usually contain arrays whose elements are objects with a field that
uniquely specifies the array entry (e.g. "name").

For example, given a JSON/YAML input document:

    {"a": [{"k":"x", "v": 42}, {"k":"y", "v": 77}]}

If "k" is a field that contains a key that uniquely identifies an element in a given array,
...
The "~" token accepts an argument which is interpreted as a JSON value to be used as a "query-by-example" filter
...
This library offers an API to retrieve multiple matches ("FindAll") or to fetch only one match and error if multiple
matches are found ("Find").
